...
func (sc SoiCup) SoisP() []*Soi {
	var sois []*Soi
	for i, _ := range sc.Sois {
		s := &sc.Sois[i]
		sois = append(sois, s)
	}
	return sois
...
func (sc *SoiCup) SetP(sois []*Soi) {
	sc.Sois = nil
	for _, s := range sois {
		sc.Sois = append(sc.Sois, *s)
	}
}

// Contains returns whether SoiCup contains name
